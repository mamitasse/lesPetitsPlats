class Controller {
  ...
  const filteredRecipesArray = Array.isArray(filteredRecipes) ? filteredRecipes : [];
  
  // Obtenir l'intersection des deux en utilisant la méthode filter
  const finalFilteredRecipes = filteredBySearch.filter((recipeBySearch) =>
    filteredRecipesArray.some((recipe) => recipe.id === recipeBySearch.id)
  );
  
  console.log("Filtered by Search:", filteredBySearch);
  console.log("Filtered Recipes:", filteredRecipesArray);
  console.log("Final Filtered Recipes:", finalFilteredRecipes);
