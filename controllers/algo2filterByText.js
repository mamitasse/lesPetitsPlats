class Controller {
  ...
  filterByText(filteredRecipes, normalizedQuery) {
    return filteredRecipes.filter((recipe) => {
        ...
        // Utilisation d'une boucle pour parcourir les champs de recherche
        for (const field of searchFields) {
            if (Array.isArray(field)) {
                // Recherche dans les ingrédients
                if (this.arrayIncludesAny(field, normalizedQuery)) {
                    return true;
                }
            } else {
                // Recherche dans le nom et la description
                if (this.arrayIncludes(field, normalizedQuery)) {
                    return true;
                }
            }
        }
...
// Méthode pour vérifier si une chaîne est incluse dans un tableau
arrayIncludes(arr, query) {
    const normalizedQuery = this.normalizeString(query);
    for (const item of arr) {
        if (this.normalizeString(item).includes(normalizedQuery)) {
            return true;
        }
    }
    return false;
...
// Méthode pour vérifier si au moins une chaîne est incluse dans un tableau
arrayIncludesAny(arr, query) {
    const normalizedQuery = this.normalizeString(query);
    for (const item of arr) {
        if (this.normalizeString(item).includes(normalizedQuery)) {
            return true;
